...
/** updateExtraHourVOs web service.
 *
 * @filesource
 * @package PhpReport
 * @subpackage services
 ...
 */
    ...
    /*$request = '<?xml version="1.0" encoding="utf-8"?><extraHours><extraHour><id>4</id><hours>100</hours><userId>1</userId><date format="Y-m-d">2012-02-01</date></extraHour></extraHours>';*/
    ...
    do {
        ...
        if (!$user)
        {
            $string = "<return service='updateExtraHourVOs'><error id='2'>You must be logged in</error></return>";
            break;
        }

        if (!LoginManager::isAllowed($sid))
        {
            $string = "<return service='updateExtraHourVOs'><error id='3'>Forbidden service for this User</error></return>";
            break;
        }
        ...
        if (count($extraHoursList) >= 1)
            ...
            {
                if (UsersFacade::UpdateExtraHour($hour) == -1)
                {
                    $string = "<return service='updateExtraHourVOs'><error id='1'>There was some error while updating the extra hour objects</error></return>";
                    break;
                }
