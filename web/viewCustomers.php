...
Ext.onReady(function(){
    ...
    var customersStore = new Ext.data.Store({
        ...
        writer:new Ext.data.XmlWriter({encode: true, writeAllFields: true, root: 'customers', tpl: '<tpl for="."><' + '?xml version="{version}" encoding="{encoding}"?' + '><tpl if="records.length&gt;0"><tpl if="root"><{root}><tpl for="records"><tpl if="fields.length&gt;0"><{parent.record}><tpl for="fields"><{name}>{value}</{name}></tpl></{parent.record}></tpl></tpl></{root}></tpl></tpl></tpl>'}, customerRecord),
        remoteSort: false,
        sortInfo: {
            field: 'name',
            direction: 'ASC',
