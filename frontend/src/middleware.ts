  ...
  if (isAccessTokenValid) {
    if (!token.user?.id) {
      return new NextResponse(
        'You do not have a user record in the application. Please contact your sysadmin.',
        { status: 401 }
      )
    }
    if (!token.user.roles || token.user.roles.length == 0) {
      return new NextResponse(
        'You have not been assigned any roles in the application. Please contact your sysadmin.',
        { status: 403 }
      )
    }
