...
function getValidUsers($init, $end) {
    ...
    */
    ...
    $validUsers=@pg_exec($cnx,$query="SELECT uid FROM users WHERE staff='t' AND  NOT (uid) IN (SELECT DISTINCT uid FROM extra_hours WHERE date>='". $init . "' AND date<='". $end . "') AND NOT (uid) IN (SELECT DISTINCT uid FROM compensation WHERE init < '" . $init . "' AND _end BETWEEN '" . $init . "' AND '" . $end ."') ORDER by uid") or die($die);
