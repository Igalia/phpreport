...
/** File for PostgreSQLProjectUserDAO
 *
 *  This file just contains {@link PostgreSQLProjectUserDAO}.
 *
 * @filesource
 * @package PhpReport
 * @subpackage DAO
...
include_once(PHPREPORT_ROOT . '/model/dao/ProjectUserDAO/ProjectUserDAO.php');

/** DAO for relationship ProjectUser in PostgreSQL
 *
 *  This is the implementation for PostgreSQL of {@link ProjectUserDAO}.
 *
 * @see ProjectUserDAO
 */
class PostgreSQLProjectUserDAO extends ProjectUserDAO{

    /** ProjectUser DAO for PostgreSQL constructor.
     *
     * This is the constructor of the implementation for PostgreSQL of {@link ProjectUserDAO}, and it just calls its parent's constructor.
     *
     * @throws {@link DBConnectionErrorException}
     * @see ProjectUserDAO::__construct()
     */
    ...
    /** ProjectUser entry retriever by id's for PostgreSQL.
     *
    ...
    /** Projects retriever by User id for PostgreSQL.
     *
    ...
    if (!is_numeric($userId))
        throw new SQLIncorrectTypeException($userId);
        $sql = "SELECT project.* FROM project_usr LEFT JOIN project ON project_usr.projectid=project.id WHERE project_usr.usrid=" . $userId . " ORDER BY project.id ASC";
    $result = $this->executeFromA($sql);
    return $result;
    }

    /** Users retriever by Project id for PostgreSQL.
     *
    ...
    if (!is_numeric($projectId))
        throw new SQLIncorrectTypeException($projectId);
        $sql = "SELECT usr.* FROM project_usr LEFT JOIN usr ON project_usr.usrid=usr.id WHERE project_usr.projectid=" . $projectId . " ORDER BY usr.id ASC";
    ...
    $affectedRows = 0;
        ...
        // Check for a project user entry ID.
        $currBelongs = $this->getByIds($userId, $projectId);

        // If it doesn't exist, then create.
        if(sizeof($currBelongs) == 0) {
        $sql = "INSERT INTO project_usr (usrid, projectid) VALUES (" . $userId . ", " . $projectId . ")";
    ...
    public function delete($userId, $projectId) {
        ...
        if(sizeof($currBelongs) > 0) {
            $sql = "DELETE FROM project_usr WHERE usrid=" . $userId . " AND projectid=" . $projectId;
...
$dao = new PostgreSQLProjectUserDAO();
